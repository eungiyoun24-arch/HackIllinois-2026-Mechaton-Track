...
// MPU6050 my_mpu6050;
DeviceDriverSet_Motor drive_motors;
DeviceDriverSet_Servo servo_controller;
DeviceDriverSet_ULTRASONIC our_ultrasonic;

float curr_time = millis();
uint16_t ultrasonic_val;
...
void setup() {
  ...
  drive_motors.DeviceDriverSet_Motor_Init();
  servo_controller.DeviceDriverSet_Servo_Init(0);
  our_ultrasonic.DeviceDriverSet_ULTRASONIC_Init();
...
void loop() {
  ...
  if (mpu.dmpGetCurrentFIFOPacket(FIFOBuffer)) { // Get the Latest packet 
      ...
      Serial.print("areal\t");
      Serial.print(aaReal.x); // - (gravity.x * 8192));
      Serial.print("\t");
      Serial.print(aaReal.y); // - (gravity.y * 8192));
      Serial.print("\t");
      Serial.println(aaReal.z); //  - (gravity.z * 8192));
      // Serial.print("\t");
  ...
  ///////////////////////////////////////////////
  // End of MPU6050 loop, start of our code.
  ///////////////////////////////////////////////

  float delta_time = millis() - curr_time;
  ...
  // Serial.println(delta_time / 1000.0);

  // Serial.print("Ultrasonic value is ");
  // our_ultrasonic.DeviceDriverSet_ULTRASONIC_Get(&ultrasonic_val);
  // Serial.println(ultrasonic_val);

  delay(200);

  // servo_controller.DeviceDriverSet_Servo_controls(1, 90);

  // delay(2000);

  // servo_controller.DeviceDriverSet_Servo_controls(1, 0);

  // delay(2000);


  // drive_motors.DeviceDriverSet_Motor_control(/*direction_A*/ 0, /*speed_A*/ 100, /*direction_B*/ 0, /*speed_B*/ 100, /*controlED*/ true); //Motor control
...
// other output styles for the MPU 6050:
